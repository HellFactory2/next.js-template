...
function validate() {
  if (!port) {
    fatalErrorHandler({ message: 'Invalid process.env.PORT (port must be a number)' });
  }
  ...
  try {
    ...
    fatalErrorHandler({ message: 'Invalid process.env.USE_ROBOT (must be true or false)' });
  }
  ...
  try {
    ...
    fatalErrorHandler({ message: 'Invalid process.env.PROXY_RULES (must be JSON)' });
  }
}
validate();
